...
pipeline {
    ...
    parameters {
        booleanParam(name: 'RUN_SAST',   defaultValue: true, description: 'Run SonarQube SAST + Quality Gate')
        booleanParam(name: 'RUN_SCA',    defaultValue: true, description: 'Run Snyk SCA')
        booleanParam(name: 'RUN_DEPLOY', defaultValue: true, description: 'Build Docker image and deploy to test env')
        booleanParam(name: 'RUN_DAST',   defaultValue: true, description: 'Run OWASP ZAP DAST (requires RUN_DEPLOY=true)')
    }
    ...
    stages {
        ...
        */
        stage('DAST - OWASP ZAP') {
            ...
            when { expression { params.RUN_DAST && params.RUN_DEPLOY } }
